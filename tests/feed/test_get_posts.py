...
def test_get_posts_ok(
    ...
    assert response.json[0].author.slug == u2.slug
...
def test_get_posts_with_offset_and_limit(
    ...
    create_post(client, u2, audio_file)
    sleep(0.001)
    create_post(client, u3, audio_file)
    ...
    response = client.get(
        "/api/v1/feed", headers=headers, json={"limit": 1, "offset": 1}
    )
    assert response.status_code == int(HTTPStatus.OK)
