...
def test_unsubscribe_on_user_that_does_not_exist(
    client: FlaskClient,
    registered_user: RegisteredUser,
) -> None:
    response = unsubscribe(client, registered_user.token, "does-not-exists")
...
def test_subscription_without_token(
    ...
    response = unsubscribe(client, None, registered_user.slug)
    assert response.status_code == int(HTTPStatus.UNAUTHORIZED)


def test_unsubscribe_ok(
    app: Flask,
    client: FlaskClient,
    registered_user_fabric: Callable[[], RegisteredUser],
) -> None:
    u1, u2 = registered_user_fabric(), registered_user_fabric()
    subscribe(client, u2.token, u1.slug)

    response = unsubscribe(client, u2.token, u1.slug)
