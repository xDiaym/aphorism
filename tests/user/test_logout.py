...
def test_logout_successfully(client: FlaskClient, registered_user: str) -> None:
    response = logout(client, registered_user)
    assert response.status_code == int(HTTPStatus.OK)
    assert response.json["message"] == "Token revoked"


def test_logout_after_logout(client: FlaskClient, registered_user: str) -> None:
    first_response = logout(client, registered_user)
    assert first_response.status_code == int(HTTPStatus.OK)
    assert first_response.json["message"] == "Token revoked"

    second_response = logout(client, registered_user)
